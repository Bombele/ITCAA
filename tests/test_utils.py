...
def test_normalize_features_numeric():
    ...
    assert all(0.0 <= f <= 1.0 for f in normalized), (
        "Toutes les valeurs doivent être normalisées entre 0 et 1"
    )
...
def test_normalize_features_mixed():
    ...
    assert len(normalized) == 2, (
        "Les valeurs non numériques doivent être ignorées"
    )
...
def test_log_prediction_creates_log_entry(tmp_path, monkeypatch):
    ...
    log_entry = utils.log_prediction(
        [0.1, 0.2], label=1, confidence=0.95
    )
    ...
    assert "Confidence=0.9500" in content, (
        "Le contenu du log doit inclure la confiance formatée"
    )