...
TEST_DIR=tests
SCRIPT_DIR=scripts
INDEXREPORT=$(PYTHONPATH)/itcaaaioffline/data/index/indexreport.md
...
verify-scripts:
        ...
        @for script in $(SCRIPTDIR)/repairindex.py $(SCRIPTDIR)/checkstructure.py $(SCRIPTDIR)/validatedependencies.py $(SCRIPTDIR)/validaterender_config.py; do \
                if [ ! -f "$$script" ]; then \
...
check:
        ...
        PYTHONPATH=$(PYTHONPATH) python $(SCRIPTDIR)/checkstructure.py || exit 1
...
index:
        ...
        PYTHONPATH=$(PYTHONPATH) python $(PYTHONPATH)/itcaaaioffline/data/corpus/index_builder.py --incremental || exit 1
...
lint:
        ...
        @mkdir -p $(LOG_DIR)
        black --check $(PYTHONPATH) $(TESTDIR) | tee $(LOGDIR)/black.log || (echo "❌ Black a trouvé des erreurs" && exit 1)
        isort --check-only $(PYTHONPATH) $(TESTDIR) | tee $(LOGDIR)/isort.log || (echo "❌ Isort a trouvé des erreurs" && exit 1)

🔎 Vérification stricte des types
typecheck:
        @echo "🔎 Vérification des types avec mypy…"
        @mkdir -p $(LOG_DIR)
        PYTHONPATH=$(PYTHONPATH) mypy --config-file=mypy.ini $(PYTHONPATH) $(TESTDIR) | tee $(LOGDIR)/mypy.log || (echo "❌ Mypy a trouvé des erreurs" && exit 1)

🧹 Nettoie les artefacts temporaires
clean:
        @echo "🧹 Nettoyage des fichiers temporaires…"
        rm -rf .pytestcache pycache /pycache .pyc .pyo .pyd *.log htmlcov/ coverage.xml $(LOGDIR)
...
docker-up:
        ...
        docker run -d --name $(DOCKERCONTAINER) -p 8000:8000 $(DOCKERIMAGE)
...
docker-test:
        ...
        docker exec $(DOCKERCONTAINER) pytest $(TESTDIR) --maxfail=1 --disable-warnings --cov=$(PYTHONPATH) --cov-report=term-missing
...
repair-index: install-faiss
        ...
        PYTHONPATH=$(PYTHONPATH) python $(SCRIPTDIR)/repairindex.py || exit 1
...
setup-prod: generate-scripts verify-scripts prod-install install-faiss repair-index check
	@echo "✅ Environnement de production prêt : dépendances installées, scripts vérifiés, FAISS installé, index réparé."
